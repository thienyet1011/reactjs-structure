...
1. src 
    - component 
        - global => ```C{filename}.js``` => Tên file bắt đầu với tiền tố "C" (Common)
        - local => ```M{filename}.js``` => Tên file bắt đầu với tiền tố "M" (Module)
    - redux 
        - global
            - action => ```{ten_action}.action.js```. VD: login.action.js
            - reducer => ```{ten_red}.reducer.js``` <=> combine của các reducer con trong global
        - local 
